-- Migration 021: Seed Marcus's documented lessons, patterns, and insights
-- Created: 2026-01-05
-- Purpose: Populate writer_memory_log with 8 key insights from Marcus's agent documentation
-- Writer: Marcus (performance coach & content creator)

-- Insert Marcus's memories
...
FROM (
  ...
  (
    ...
    'Avoid words that signal AI writing: "delve", "robust", "leverage", "essentially", "furthermore", "utilize", "paradigm", "synergy", "holistic". Instead use natural conversational alternatives: "dig into", "strong/solid", "use", "basically", "also", "use", "shift", "work together", "complete". Marcus''s voice is direct and punchy, not corporate. Read drafts aloud to catch unnatural phrasing. This keeps his authentic performance-coach voice consistent.',
  ...
  (
    ...
    'Shorter sentences and simpler words work better for performance audience. Grade 8-10 reading level means most of the carnivore community can engage without friction. Avoid multi-clause sentences, technical jargon without explanation, and unnecessarily complex vocabulary. Shorter paragraphs, concrete examples, and active voice make content more scannable and actionable. Short punchy sentences are Marcus''s signature—they''re easier to read AND more impactful.',
  ...
  (
    ...
    'Use Marcus''s signature phrases naturally: "Here''s the protocol...", "The math doesn''t lie...", "Stop overthinking it...", "This is why it works...", "Next, you do this..."',
    'These five phrases anchor Marcus''s voice and signal his performance-coach authority: (1) "Here''s the protocol..." introduces actionable systems. (2) "The math doesn''t lie..." grounds advice in data/metrics. (3) "Stop overthinking it..." removes barriers and builds confidence. (4) "This is why it works..." explains mechanism clearly. (5) "Next, you do this..." gives clear action steps. Use each naturally in content flow, not forced. These phrases make content feel authentically Marcus—direct, metric-driven, and action-oriented.',
  ...
  -- 5. Metrics requirement (specific numbers, not "many")
  (
    'pattern_identified',
  ...
  -- 8. Action steps must be numbered and clear
  (
    'lesson_learned',
    ...
    'Protocols live in clarity. Never list action steps without numbers—numbering signals progress and makes them scannable. Format: "1. [Action] (detail). 2. [Action] (detail)." Example: "1. Buy rib eye steaks ($8/lb), portion into 6-oz servings. 2. Season with sea salt only. 3. Pan-sear 4 minutes each side. Expected: cooked through, no carryover." Include quantity, cost, time, or expected result for each step. Vague steps fail. Readers need to know what success looks like. If a step is unclear when you write it, it''ll confuse readers. Test the protocol yourself first—if you can''t follow your own instructions, they need rewriting.',
...
-- Verify insertion
SELECT
  'Marcus Memory Seeding Complete' as status,
  COUNT(*) as total_memories_for_marcus,

...
(async () => {
  ...
  try {
    ...
    await page.locator('select[name="heightFeet"]').selectOption('6');
    await page.locator('select[name="heightInches"]').selectOption('0');
    ...
    await page.locator('select[name="currentDiet"]').selectOption('carnivore');
    ...
    // Check brand colors
    const headingColor = await page.locator('h2:has-text("Your Personalized")').evaluate(el =>
      window.getComputedStyle(el).color
    );
    console.log('\n🎨 Brand Colors:');
    console.log('  Heading:', headingColor, headingColor.includes('255, 215, 0') ? '✅' : '⚠️');

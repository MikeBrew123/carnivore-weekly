...
**Weekly cadence.** Generation and publication are two different schedules; do not conflate them.
- **CW: 9 posts/week.** Generated by the `weekly-blog-content-generation` scheduled task, Sun + Wed 4:30am.
- **KD: 2 posts/week, landing Tuesday and Friday.** Generated by the `kd-blog-content-generation` scheduled task on **Saturday** 4:33am, which writes both of the coming week's posts in one run. **Approved by Brew 2026-08-09 and capped deliberately:** ketodial.com is a dropped-and-re-registered domain that Google has algorithmically shelved, and on an untrusted fresh domain more pages reinforce the scaled-content pattern instead of earning crawl demand. **Do NOT raise the batch size, compress the spacing, or add a catch-up run for a missed week.** If you think the cadence should change, escalate to Brew. Nothing downstream enforces this: `daily_publish.py` publishes every `ready` post whose date has arrived, with no per-site cap, so the generation step is the only control.
- **Publication** is separate and daily: the `daily-publish.yml` GitHub Action at 9 AM EST runs `scripts/daily_publish.py`, publishing every post with status `ready` and `publish_date <= today`. It does not create posts and does not know about either cadence.
- Manual top-up: follow `scripts/weekly_content_prompt.md`. `scripts/autonomous_blog_generation.sh` blocks on stdin and cannot run unattended.
...
**Full reference: [`docs/guides/email.md`](docs/guides/email.md).** Addresses, endpoints, tables, drip and newsletter procedures, quota handling and the runtime source of truth for each all live there. Read it before any email work. The binding rules are repeated here because each one reaches a subscriber when it is broken:
- **Beehiiv and MailerLite are DEPRECATED (purged 2026-07-04 / 2026-05-26). Never use either.**
- **Never write drip copy claiming a deadline Stripe does not enforce.** Day-7/28 sends mint single-use 48h promo codes; mint failure falls back to a static code with no expiry claim. The claim follows the code, never the reverse.
- **Every subscriber send replies to an `@carnivoreweekly.com` catch-all, never a personal inbox** (never `iambrew@gmail.com`). Inbound mail is swept daily by `writer-inbox-daily-check`. **Four different reply-to addresses are in use and they are not uniform per site**: the KD drip replies to `ketodial@`, but the KD newsletter and KD Coach newsletter reply to `newsletter@`, and coach-launch replies to `sarah@`. Do not generalise from one path to another; the per-path mapping is in the guide. KD promo copy says "enter code at checkout", never "auto-applies".
- **Resend is on a PAID plan (Brew, confirmed 2026-09-13).** The old free-tier 100/day cap no longer applies, and a full newsletter to the whole list is fine in one send. Do not split sends or hold back a newsletter on quota grounds, and do not change the plan without Brew.
- **A quota-refusal 429 is never retried.** `daily_quota_exceeded`/`monthly_quota_exceeded` writes a row to Supabase `email_send_refusals`, opens a GitHub issue and turns the run red. Drip refusals retry themselves next run; **newsletter refusals do not**, so a missed newsletter stays missed until someone sends it. A `rate_limit_exceeded` 429 still backs off and retries.

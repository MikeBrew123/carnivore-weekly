(function() {
    ...
    const RATE_LIMIT_KEY = 'cw_refund_last_submit';
    ...
    function init() {
        ...
        // Any element with data-open-refund-modal opens it (e.g. a footer link)
    ...
    function updateCharCount() {
        ...
        charCount.style.color = length > 950 ? '#ff6b6b' : '';
    }
    ...
    async function handleSubmit(event) {
        ...
        const adaptationGuideFeedback = getRadioValue('adaptation_guide_feedback');
        ...
        if (!email) {
            alert('Please enter the email you used for your purchase.');
            return;
        }
        if (!reasonCategory) {
        ...
        if (!overallRating) {
            alert('Please rate the overall quality 1-5.');
            return;
        }
        if (!mealPlanFeedback || !doctorScriptFeedback || !adaptationGuideFeedback) {
            alert('Please rate all three report sections.');
            return;
        }
        if (reasonText.length < 10) {
            alert('Please tell us a bit more (10 characters minimum).');
            return;
        }
        if (reasonText.length > 1000) {
            alert('Please keep your explanation under 1000 characters.');
            return;
        }

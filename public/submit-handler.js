...
function showProgressBar() {
  ...
  if (wrapper) {
    ...
    // Start the 5-stage animated progress when showing the bar
    startReportGenerationAnimation();
  }
}
...
/**
 * Animated 5-stage progress bar for report generation
 * Simulates the actual report generation process with realistic timing
 * Keeps customer engaged and informed during the 20-30 second generation
 */
function startReportGenerationAnimation() {
  const stages = [
    { progress: 20, label: 'Calculating macros...', duration: 4000 },
    { progress: 40, label: 'Analyzing your health profile...', duration: 5000 },
    { progress: 65, label: 'Generating personalized protocol...', duration: 6000 },
    { progress: 85, label: 'Building your food guide...', duration: 6000 },
    { progress: 100, label: 'Creating your meal plan...', duration: 5000 },
  ];
  ...
  function runNextStage() {
    if (currentStageIndex >= stages.length) {
      // All stages complete
      console.log('Report generation animation complete');
      return;
    }

    const stage = stages[currentStageIndex];
    updateProgressBar(stage.progress, stage.label);
    currentStageIndex++;

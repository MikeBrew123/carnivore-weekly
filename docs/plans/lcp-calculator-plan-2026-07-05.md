...
- Live post-deploy LCP: **committed (54f5d5a7) and pushed, deploy confirmed live** (curl shows `CarnivorFP-480.webp` served) -- BUT live re-measurement is inconsistent/concerning, see note below. 7 runs: 5141, 2932, 5065, 4999, 2214, 4868, 5389ms. Median of 7 = 4999ms, i.e. WORSE than the pre-fix live baseline median of 2400ms.
- Date/session: 2026-07-05, Sonnet-tier execution session

**Live post-deploy measurement -- flagged, not resolved:**
The 7 live post-deploy runs are strongly bimodal: ~1.1s FCP / 2.2-2.9s LCP on 2 runs, vs ~3.1-3.3s FCP / 4.9-5.4s LCP on 5 runs. Same H1 LCP element throughout (image change did not shift the LCP candidate). The `lcp-breakdown-insight` audit attributes the slow runs to a large "element render delay" (870-1541ms) but the phase breakdown sums (TTFB+delays) don't add up to the full reported LCP in ANY run here, live or local, so that audit looks unreliable on this page/Lighthouse version -- not trustworthy for root-causing.
Important context: this same bimodal pattern (one 5.5s outlier vs two 2.3-2.4s runs) was ALSO present in the Phase A pre-fix baseline, i.e. the live site already had this flakiness before any of today's changes. A 15s cooldown between runs did not fix it (run7 was still slow), so it isn't simple back-to-back CPU contention from rapid Lighthouse invocations either.
Given: (a) the change only affects downloaded byte counts for the pyramid images, which load in parallel with/after the H1 and shouldn't be able to delay H1 paint timing; (b) the local git-stash A/B eliminated all confounds and showed a clean, repeatable 18.7% improvement with no bad runs; (c) the same instability pre-existed the fix -- the most likely explanation is real-world network/edge variance on carnivoreweekly.com (Cloudflare Pages edge cache state, or variance in this test machine's route to it) rather than a regression caused by this change. This is a hypothesis, not a confirmed root cause.
**Recommended before treating this as done:** re-run live Lighthouse from a different network/time of day, or use PSI/CrUX field data (real-user, not lab) once the anonymous quota resets, to get a signal that isn't confounded by this machine's live-network noise. Do not use this session's live numbers as the final verdict.

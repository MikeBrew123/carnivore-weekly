...
If recurs: FIRST check which worker the Resend webhook actually targets (GET https://api.resend.com/webhooks) vs where the fix was deployed — bare and -production are separate workers and BOTH must be deployed until the webhook is repointed to production. Then verify the partial unique index still exists (pg_indexes on drip_events).

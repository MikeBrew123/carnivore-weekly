...
🟢 FIXED (guard implemented 2026-07-19) | Last: 2026-07-19
...
- 2026-07-19 — GUARD IMPLEMENTED (after the 4th occurrence). Two-layer fix: (1) ROOT CAUSE — `content_analyzer_optimized.py` `recent_posts` now filters `site=="cw"`, so KD posts are never offered to the CW editorial LLM (this was the leak, same class as ISSUE-041 cause 1). (2) BACKSTOP — new `scripts/blog_link_guard.py::sanitize_cw_blog_links()` strips any `/blog/<slug>.html` link (markdown OR html) whose file isn't live under `public/blog/`, keeping the anchor text. Applied to `weekly_summary` in BOTH content_analyzer (before persisting to analyzed_content.json) and generate.py (before rendering public/index.html — last line of defense, catches hallucinated slugs too). Cleaned the stale bipolar link out of analyzed_content.json. Unit-tested against the real failure case (markdown + html). Validator 0 critical.
If recurs: the guard failed or a NEW path emits /blog/ links (newsletter/roundup image/etc.) — extend `sanitize_cw_blog_links` to that path rather than hand-patching. But this should now be closed for the homepage editorial path.
...
- 2026-09-12 — RECURRED, different trigger. CW drip exited 1: "SAFETY STOP: 89 subscribers exceeds cap of 50." Cap should have been ~114 (3x the 31-38/day CW was actually sending); it returned MIN_SEND_CAP. Cause: `dynamic_send_cap()` ends in `except Exception: return MIN_SEND_CAP`, and Supabase was failing in that same run — the KD drip crashed 10s later on a 504 Gateway Timeout (GH #62). A transient DB error is therefore silently converted into a whole-day drip outage, reported to the operator as a "bad-data signup flood". Self-recovered 09-13 (41 sent). GH #61/#62 closed 2026-09-14; fix filed as a bead, not applied.
...
- 2026-07-23 — Recurred as "KD 0% delivery on 7 sent" in Command Center. TWO causes: (1) the Resend webhook URL points at the BARE worker `carnivore-report-api.iambrew.workers.dev`, but the 07-18 fix + KD site attribution only deployed to `carnivore-report-api-production` — so the live webhook was stale code: re-logging 'sent' AND stamping every event site='cw' (KD's 7 emails were all delivered+opened, just filed under cw). (2) The unique index from 07-18 was GONE (likely lost around the 07-20 site-column migration), so dups inserted silently. Fixed: hardened site attribution (tags arrive as [{name,value}] array not object; added coach@ → kd), deployed BOTH workers, retagged 20 KD events cw→kd, deleted 27 dup 'sent' rows, restored index via migration restore_drip_events_singular_event_unique_idx. Post-fix 7d: KD 7/7 delivered (100%), CW 174/178 (97.8%).
- 2026-07-23 (later) — Repointed the Resend webhook (PATCH, same ID so signing secret preserved) to carnivore-report-api-production/webhook/resend. Stripe webhooks audited: CW already on -production; coach.ketodial.com + ketodial-api are separate and intentional. Verified end-to-end with a live test send — delivered event logged via new endpoint, correct site, no dup sent. Bare worker left deployed (old emails may link it) but nothing depends on it anymore; only --env production deploys needed from now on.
If recurs: verify the webhook target (GET https://api.resend.com/webhooks) still points at -production, then verify the partial unique index still exists (pg_indexes on drip_events).
...
- 2026-07-22 (d recurred) — the (d) fix kept reverting because CI lacked Pillow: daily-publish.yml and weekly-update.yml install deps via inline pip lists that omitted it, and both generators degrade silently without PIL (get_post_image_dims() / _get_image_dims() fall back to hardcoded dims on ImportError). Every CI regeneration rewrote 1200x630/360x240 into the HTML. Fixed (commit 10cce563, Brew-approved via 2026-07-21 journal): pillow>=10.0.0 added to both workflows' pip installs + promoted from "optional" to real dependency in requirements.txt. The ~180 already-reverted dims in checked-in blog HTML self-heal on the next scheduled CI regeneration. If recurs again: check the workflows' pip install lines still include pillow, and that no new workflow regenerates pages without it.
...
- 2026-07-30 — RECURRED: 3 more posts queued before/during the 7/27 gate landed (`2026-07-27-liver-organ-dosing` 172 chars, `2026-07-28-carnivore-influencers-plants` 182, `2026-07-29-regenerative-meat-sourcing` 194) hit the write-time gate and were correctly blocked from rendering — but `generate_blog_index`/`update_sitemap`/`find_related_posts` in `generate_blog_pages.py` filter only on `post["published"]`, not on whether the individual post file actually wrote successfully. So the blocked posts still got linked from the homepage, blog index, sitemap, and 4 unrelated posts' related-content sections, tripping `validate_before_commit.py` as broken-link criticals. This exited `daily_publish.py` with an error every day 7/27–7/30, and because `deploy` job `needs: publish` in `daily-publish.yml`, **the site did not deploy for 3 days**. Fixed by trimming all 3 descriptions to 144/162/163 chars and re-running publish → 0 critical → pushed (f80cb05b). GitHub issue #53 (auto-filed by the health-check job) closed.
If recurs: same as before (check for a new path bypassing generate_blog_pages.py / validate_only) — AND check whether the real gap has been closed yet: generate_blog_index/update_sitemap/find_related_posts should only link posts that were actually written to disk (e.g. track a `written_slugs` set in generate_blog_posts() and pass it through), not just `post["published"]`. That structural fix has NOT been made yet — this Attempt was a manual data fix, not a code fix, so the gate + broken-link cascade combo can recur with any future over-length post.

## ISSUE-060 — Meal-plan PDFs shipped with overprinted grocery/directory pages
🟢 FIXED — Last: 2026-07-27
Pattern: build_mealplans.py used fixed-height `.page` divs + balanced CSS columns; any grocery list or meal directory taller than one page silently overprinted the next page. Keto, lowcarb, pescatarian, and mediterranean PDFs (listed Jul 19-20) all shipped broken; carnivore and lion happened to fit. Found while investigating zero views on the 6 new listings (bead pshn) — every preview page had to be screenshotted, which exposed the overlap.
Attempts:
...
- 2026-08-10 (weekly ops) — Status correction: that third attempt was UNCOMMITTED in the working tree and therefore undeployed. Evidence: gmail.vom and yagoo.com still bounced Aug 8-9, and both subscriber tables showed 0 rows with `bounced_at` set against 12 bounced events in 7 days.
- 2026-08-10 — NOW SHIPPED. Bounce suppression deployed as worker version 78b45803 (verified live: 3 synthetic Transient bounces suppressed on exactly the 3rd, both tables). Signup validation deployed as version 629cb716 (verified live: gmail.vom and yagoo.com both 400 with correct suggestions). NOTE: the first live test ran seconds after deploy and hit the stale edge version, returning 200 and inserting 4 rows that were then deleted. Wait ~30s after `wrangler deploy` before smoke-testing.
- 2026-08-11 — Independently re-verified on live worker 80050dc1 (bead 87gx): Permanent bounce suppressed on first hit, 3 Transient bounces suppressed on exactly the 3rd (`bounced_at` = 3rd event's timestamp, not the 1st), both tables patched, site correctly inferred as kd from the ketodial@ sender, and the real `send_drip.py` pending query excluded the address. All test rows deleted. Gap found while testing: the endpoint accepts UNSIGNED posts, so the suppression path is remotely triggerable against any address — bead carnivore-weekly-c4w2 (P1).
...
- 2026-08-10 — Follow-up: meal plan portions were protein-anchored only → lean-fish days ~1,000 cal under, dense cuts ~600 over. Added rotation density floor (≥150 cal/100g) + per-meal calorie ceiling + butter scaling 1-4 tbsp. 30-day spread now 1,925-2,156 vs 2,199 target. Brian's report regenerated, DB row updated, apology sent with corrected PDF (Resend f4e77979).
...
## ISSUE-070 — Resend webhook accepted unsigned POSTs (remote kill switch for all email)
🟢 FIXED — Last: 2026-08-11
Pattern: `handleResendWebhook` parsed `request.json()` and acted on it with no signature check, while the Stripe handler in the same file verified properly and `resend.webhook_signing_secret` sat unused in secrets. Harmless when the endpoint only logged analytics; ISSUE-067's suppression change turned it into a remote kill switch — one forged `bounce.type: Permanent` POST silently stops all email to any address on either site, and nothing alerts because suppression is working as designed.
Attempts:
- 2026-08-11 — Found while verifying ISSUE-067 (bead 87gx): unsigned POSTs from a laptop wrote `drip_events` rows and mutated `drip_subscribers.bounced_at` + `newsletter_subscribers.status` on production.

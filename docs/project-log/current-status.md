...
| Day | State | Live content |
|-----|-------|--------------|
| 2 | Live, hotfix done (`187d2493`) | Four-metric grid. Remains the longitudinal baseline. False anonymity claim, retired aggregate-result promise and unqualified sodium guidance all removed. |
| 3 | **FROZEN pending real-user data** (`826a8cc3`) | Subject "What to buy this week, and what to leave on the shelf". Marcus's email verbatim with both sodium qualifications; Sarah's `symptoms_today` payback on `journey-checkin.html`; eight existing options; render-only contradiction handling; no WELCOME5 sales push; mobile Rule 7 verified at 320px. |
| 5 | **FROZEN pending real-user data** | `goal_target`, six banded one-tap options, calculator-backed horizon payback, change-my-answer. CW only. |

**Infrastructure shipped this sprint (all deployed, all on main):** v1 analytics baseline freeze; check-in identity join (opaque `checkin_token`, case-insensitive calculator lookup); goal-magnitude to horizon payback engine; longitudinal trend engine (`computeCheckinTrend`, sender-only, deliberately NOT wired to any HTTP route); day-5 eligibility and skip rule; privacy disclosure correction on `journey-checkin.html`.

**KD is deliberately untouched.** KD day 5 is still the four-metric grid; its `goal_target` and six options are seeded and INACTIVE; its eligibility rule is dormant. Do NOT activate CW day 5 logic on KD. KD day 5 copy is `carnivore-weekly-kwjf`; the sender transition is its own backlog item.

**Day 3 longitudinal break, expected, do not repair.** Nine historical `symptoms_today` rows exist (2026-07-29 to 2026-09-09), all with `subscriber_id = null`. New tokenized answers join to a subscriber; those nine cannot. Historical anonymous answers stay anonymous.

**Day 3 verification correction, for the record.** The Day 3 implementation report first stated the option submissions were tested against the production worker. That was wrong. All Day 3 test submissions hit a local stub (`localhost:8787`; the page routes by hostname). Zero Day 3 rows were written to production during testing, production was queried read-only afterward, and there is no synthetic data to delete.

**Next read:** `carnivore-weekly-u05p`, day-5 first-send results after the 2026-09-13 09:00 EST cron. Watch `drip_events` `skipped` plus `skip_reason` (29% of active subscribers, 37 of 125, have an explicit maintain or gain goal), the `goal_target` band distribution, and `answered_via='one_tap'` versus page answers.

**2026-09-10 · CW Mobile Visual Quality + Accessibility is CLOSED and frozen.** Four production milestones, each verified live at 375/390/430 with Pages and calculator-guard green: Mobile Step 1 `aad6d117` (unit switch converts instead of clearing, visible associated height errors, 182 cm no longer fails silently); Batch 1 `d52168be` ($29 offer trust and plan text at 15px, about 5.9:1); Batch 2 `b39e5040` (calculator first screen fits the phone, Start Calculator no longer covers the hero); Batch 3 `806b7a3b` (payment window: Pay visible on open, price 16.8:1, real dialog with focus trap, Escape and a 44px named close, coupon behind a disclosure; request bodies, redirects, 422 messages and analytics unchanged). Do not reopen any of them without production evidence of a violation, and there is no Batch 4. **Naming check:** after Batch 3 a correction was proposed saying the payment window shows "Your 30-Day Meal Plan" via `PricingModal`. Checked against `806b7a3b`, it does not hold: `CalculatorApp` renders `StripePaymentModal` directly with `tierTitle="Complete Carnivore Protocol"`; `PricingModal` (where `bundleOption.title = 'Your 30-Day Meal Plan'` lives) is imported only by the unmounted legacy `CalculatorWizard`; the deployed bundle `index-pHGnYWEs.js` contains "Complete Carnivore Protocol" and not "Your 30-Day Meal Plan"; the live render shows heading "Complete Payment" and product "Complete Carnivore Protocol" beside the "Get My 30-Day Plan for $29" CTA. No naming change was made; aligning it would be a separate decision. **Backlog, needs new evidence and a new approved project:** article link colour and CTA depth; floating chat and menu buttons over content site-wide; Deficit Target wording; tablet hero overflow; desktop hero sizing; 430px floating button over the next section. Future CW work starts from a new business objective with its own frozen definition of done.

**2026-09-10 · CW Revenue Readiness Batch 1 is LIVE and frozen.** Merge `34e47842` on main (branch `fix/revenue-readiness-batch1` @ `d2a8a996`, preserves `61fbc5fd` and `62e21ef3`), worker `carnivore-report-api-production` version `2f647a50-9803-4be6-8de6-4ae9582fe149` at 100%, Pages deploy and calculator-guard CI green. Eight frozen requirements, all verified on production: (1) diet-based signup routing unchanged (`subscribeCore`, `handleUnsubscribe` byte-identical; `scripts/` zero diff) with Step 1 now saying "Enter your email to continue. Your results appear here in the calculator. Using it also signs you up for free emails matched to the diet you choose..."; CW does NOT email free results (`/api/v1/calculator/email-report` needs a paid report row); (2) "No signup required" and "results emailed" claims removed from the blog CTA, desserts sidebar, Step 1 and the keto post source in `blog_posts.json`; (3) P0 fixed: payment modal rendered conditionally, not in `AnimatePresence`, which had left an invisible z-index 10000 overlay swallowing clicks after close; (4) sample day built by `api/sample-day.js` from ingredient lists scaled to the FINAL target, calories derived from food, worst drift 3.1% over 800-4000 kcal; (5) 18+ gate client and server (`checkTargetEligibility` on `/create-checkout`, `/report/init`, `/payment/initiate`, fails closed on missing age); (6) self-service fat-loss floor female 1200 / male 1500, inline and identical in `calculateMacrosCanonical` and worker `calculateMacros` (parity 1495/1495, golden regenerated), capped targets explained in calories, targets at or below maintenance return null macros and block sample day, offer and report; (7) retention copy names no window (code writes `expires_at` 365 days), "yours forever" and "48 hours" removed; (8) suites `calorie-guardrail`, `sample-day-arithmetic`, `signup-and-retention-copy`, `payment-modal-dismiss`, each mutation-tested, run in calculator-guard. 8 tagged QA rows backed up and deleted. Backlog, deliberately not chased: 8 stale pre-fix bundles in `public/assets/calculator2/assets/` still served to cached `calculator.html`; parity test compares 4 of 11 fields; `strict:false` so nullable macro types are unenforced. Next phase belongs to a new chat: audit-only mobile polish, readability, trust and conversion pass for 45-70.

**2026-09-10 — KetoDial paid report v1 is LIVE. Four launch blockers, audited, fixed, reviewed against the final SHA, released as PR #59 (`ddf304fa`), worker `99e89cb9` at 100%.** A read-only audit froze the definition of sellable and found exactly four blockers; everything else went to backlog untouched. (1) **Renal/adrenal** — `blob.includes('renal')` matched "adrenal", so a customer who answered No to the kidney question and typed "hydrocortisone for adrenal insufficiency" had their protein target suppressed, the meal plan and both bundles pulled from sale, and a Doctor's Report telling their physician they had declared kidney disease. Now a matching rule per term: `renal` refuses the two prefixes that move the word to a different organ, the kidney compounds (prerenal, perirenal and four more) are terms of their own, and invisible Word/PDF paste characters are stripped first (`b5a96493`, 281 assertions). (2) **Diabetes medication** — the report keyed its glycemic caution and its whole medication table off the condition chips, so an insulin user who ticked nothing got an 18g carb target, "None reported", and no mention of hypoglycemia; the intake form has no Type 1 option, so that box is the only place they can tell us. One canonical signal now feeds every gate: insulin/sulfonylurea keep their targets and gain a prescriber warning in all three reports, SGLT2 has the ketogenic targets withheld everywhere with the meal plan and bundles withdrawn from sale, neither is called diabetic (`a0d0c3c9`, 410 assertions). (3) **Meal plan** — `scaleMeal()` multiplied the macro numbers and copied the description, so a customer on 890 kcal was told to cook a 442-kcal plate beside a column reading 265; cooking the written week delivered ~1,480 kcal against an 890 target. A meal is now an ingredient list: the food is scaled, rounded to something measurable, and the macros summed from what was rounded to, so the printed numbers are by construction what the printed food delivers. Grocery list derived from the generated week. Days land within 8% of calories and 18% of protein across the full 800-6,000 kcal range (`359bb41b`, 26,500 assertions). (4) **Mobile** — `.page{width:8.5in}` with `@media print` as the only media query in the stylesheet, so every paid report rendered 816px wide at every viewport and a phone showed 48% of the page; the delivery email's only instruction is to open them in a browser and there is no attached PDF. One `@media screen` block appended after the per-report CSS, grids collapse, nothing hidden, print pagination unchanged at 4/7/5 (`b914ee07`, 838 assertions in real Chromium). **Production QA:** five tagged personas against the live worker, no charge — adrenal not renal and all five products offered; "stage 3 ckd" suppressed with a 409; Jardiance blocked with the medication-specific decline message; insulin allowed; metformin unaffected. QA rows deleted (0 remaining), 21 pre-existing KD sessions untouched. **Method notes worth keeping:** reviewers must review the FINAL SHA (three times a reviewer passed a snapshot the code then moved past, and each was re-reviewed before closing), and mutation probes must themselves be verified — several reported success while testing the wrong element. **FROZEN:** do not reopen any of the four without a production failure. Residuals recorded in decisions.md and filed as beads.


**2026-09-14 CORRECTION to the 2026-08-25 entry below (Brew).** The coach was **never launched. It was not ready.** No promotion and no email of any kind ever went out, the 08-27 send described below did not happen, and nobody signed up. Verified 2026-09-14: `coach_members` holds 9 rows, every one `status='test'`; zero real members, zero paid; Stripe has never seen a $49 charge; COACH50 zero redemptions. The "September cohort" and its Sep 15 start were internal notes, never a commitment to anyone. **Treat the coach as shelved, not as late or at risk**, and do not raise it as an overdue deadline in a weekly review. Detail in the `project-carnivore-coach` memory file.

**2026-08-25 — Carnivore Coach went from dead to selling in one day; send 08-27.** The coach was not neglected, it was non-functional: no production build had EVER succeeded, emailed sign-in links could not work by construction, Supabase Site URL was still `localhost:3000`, and the only waitlist row was Brew's own. Eight launch blockers found by walking the buyer journey, not by reading code: (1) `.gitignore` excluded `package-lock.json` repo-wide → Vercel installed stripe 22.5.0 against a pinned 22.2.0 apiVersion → every build failed on a type error; committing that one lockfile turned the pipeline green. (2) Payment Links leave `session.customer_email` null (it lands on `customer_details.email`), so the webhook created no account. (3) Webhook wrote `site='cw'` / `tier='carnivore-6wk'`, both violating check constraints (`site` IN ketodial|carnivoreweekly, `tier` IN weekly|daily). (4) **No emailed sign-in link of any type worked** — Supabase issues recovery/magiclink/invite on the implicit flow with the session in the URL *fragment*, which `/auth/callback` (PKCE `?code` only) cannot see; combined with passwordless webhook-created users, every buyer would have been permanently locked out. Fixed on the login page, which now reads the fragment and routes by role. (5) Supabase Site URL + empty redirect allow-list. (6) New members greeted "CHECK-IN OVERDUE" on day one. (7) Coach never saw the member's own "what does success look like" answer (written to `goal_target`, read from `success_vision`). (8) Thread page rendered an empty shell on any load failure. **Shipped alongside:** $49 six-week product sold via Stripe Payment Link `plink_1U8QKBEVDfkpGz8w…` (seat cap 40, deliberately NOT routed through `handleCreateCheckout` which would pollute `calculator_sessions_v2`); post-purchase welcome email + owner sale alert wired into the webhook; COACH50 (50% off the Complete Protocol); money-back guarantee; `coach-welcome.html`; announcement post queued for 08-26. **`CRON_SECRET` was never set** so the reminder cron had 401'd since it was built — zero reminders ever sent; setting it surfaced 3 notifications aimed at `@test.ketodial.com` (hard bounces), now guarded. **"Coach Remy" retired** across 29 refs + system prompt (v1→v2, now forbids inventing a name/backstory) — the coach is a role, timed before any real member existed. **Verified end to end with a real $0 purchase:** checkout → account → sign-in link → 3-step onboarding (26/31 fields, `risk_level` auto-escalated to `caution` off medications) → dashboard → check-in → coach reply → email in a real inbox. **Send 08-27 to 139 unique recipients** (corrects the "190" used all quarter: that was `drip_subscribers` across BOTH sites incl. unsubscribed; real CW reach is 137 newsletter + 132 drip overlapping by 130). Dedicated `scripts/send_coach_launch.py`, no default that sends. **Forecast: 2-5 buyers ($98-245). This cohort does NOT reach $1k/mo** — that is a recurring tier, ~53 members at $19. **Watch:** first real purchase (only thing not yet proven by a paying customer); nav link still undecided; push email 09-03.

...
export function buildMedicalContextBanner(ctx) {
  ...
  const lines = [
    ...
    '>',
    // List items, not three consecutive quoted lines. Consecutive lines inside a
    // blockquote render as ONE paragraph, and HTML collapses the newlines, so the
    // reader saw "Conditions you reported: None reported Symptoms and concerns you
    // reported: pelvic floor prolapse Medications you reported: None reported" as a
    // single run-on line. These are three separate facts about the reader and have to
    // read as three rows.

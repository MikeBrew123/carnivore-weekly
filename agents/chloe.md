...
>
> This is **not** the live `chloe` definition and the Agent tool never loads it.
> The live persona is `.claude/agents/chloe.md`, invoked by `subagent_type`, not by reading a file.
>
...
> section into `.claude/agents/chloe.md` instead, which is a separate decision.

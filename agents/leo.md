...
>
> This is **not** the live `leo` definition and the Agent tool never loads it.
> The live persona is `.claude/agents/leo.md`, invoked by `subagent_type`, not by reading a file.
>
...
> section into `.claude/agents/leo.md` instead, which is a separate decision.

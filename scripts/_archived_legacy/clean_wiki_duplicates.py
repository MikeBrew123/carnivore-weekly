...
def clean_wiki_duplicates(wiki_path):
    ...
    video_div_pattern = (
        r'<div style="margin: 20px 0; padding: 15px; background: #f5f5f5; border-radius: 8px;">'
        r"\s*<strong>Related videos this week:</strong>.*?</div>\s*"
    )
    ...
    placeholder_count = cleaned_content.count(
        "Related videos will be inserted here by automation"
    )
...
def main():
    ...
    original_count, cleaned_count, placeholder_count, success = clean_wiki_duplicates(
        wiki_path
    )
    ...
    if success:
        ...
        print("   Wiki is now ready for fresh video injection")
        print()
        return 0
    else:
        print("⚠️  Cleanup did not complete successfully")

...
Focus on peer-reviewed research. If no direct studies exist, cite related research on low-carb/ketogenic diets."""
            ...
            if start != -1 and end > start:
                try:
                    ...
                    answer_match = re.search(
                        r'"answer"\s*:\s*"([^"]+(?:\\.[^"]*)*)"', response_text
                    )
                    if answer_match:
                        answer_text = answer_match.group(1).replace("\\n", "\n")
                        answer_data = {"answer": answer_text, "citations": [], "caveats": ""}
                    else:
    ...
    def add_qa_to_analysis(self):
        ...
        # Extract questions from analysis data
        # Supports both legacy nested structure and current flat structure
        questions = []

        # Legacy path: data["analysis"]["community_sentiment"]["common_questions"]
        if "analysis" in data:
            questions = (
                data.get("analysis", {}).get("community_sentiment", {}).get("common_questions", [])
            )

        # Current flat structure: derive questions from trending_topics
        if not questions and "trending_topics" in data:
            print("   Using flat analyzer output — generating questions from trending topics")
            try:
                ...
                for topic_obj in topics:
                    ...
                    if topic_name:
                        questions.append(
                            f"What should people know about {topic_name.lower()} on a carnivore diet?"
                        )
            except (json.JSONDecodeError, TypeError, KeyError) as e:
                print(f"   ⚠️ Could not parse trending_topics: {e}")
        ...
        # Add Q&A to analysis (supports both nested and flat structures)
        if "analysis" in data:
            data["analysis"]["qa_section"] = qa_list
        else:
            data["qa_section"] = qa_list

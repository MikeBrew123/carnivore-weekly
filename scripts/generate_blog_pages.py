...
def setup_jinja():
    ...
    env.filters["rss_date"] = lambda date_str: (
        datetime.strptime(date_str, "%Y-%m-%d").strftime("%a, %d %b %Y 00:00:00 GMT")
        if date_str
        else ""
    )
...
def generate_blog_posts(env, posts, validator=None):
    ...
    for post in published_posts:
        ...
        if not content or content.strip() == "":
            ...
            print(
                f"   This post is marked published but has no content. Set published: false in blog_posts.json"
            )
            continue
...
def generate_blog_index(env, posts):
    ...
    rendered = template.render(blog_posts=published_posts, total_posts=len(published_posts))
...
def update_sitemap(posts):
    ...
    for url_elem in root.findall("sm:url", ns):
        ...
        lastmod = (
            url_elem.find("sm:lastmod", ns).text
            if url_elem.find("sm:lastmod", ns) is not None
            else "2026-01-01"
        )
    ...
    for url, file_path in main_pages:
        ...
        if full_path.exists():
            ...
            if url in url_data:
                ...
                url_data[url] = {"lastmod": lastmod, "changefreq": "weekly", "priority": priority}
...
def generate_rss_feed(env, posts):
    ...
    build_date = (
        published_posts[0].get("date", "")
        if published_posts
        else datetime.now().strftime("%Y-%m-%d")
    )

    rendered = template.render(blog_posts=published_posts, build_date=build_date)
